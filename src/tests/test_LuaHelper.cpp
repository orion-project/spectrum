...
#define ASSERT_LUA_CALC(lua, code, expected_value) {\
    ...
    ASSERT_EQ_DBL(res.result(), expected_value) \
}
...
#define ASSERT_LUA_EXEC(lua, result_var, expected_value) {\
    ...
    ASSERT_EQ_DBL(val.result(), expected_value) \
}
...
#define ASSERT_LUA(val, e1, e2) { \
    ...
    ASSERT_EQ_DBL(r1.result(), r2.result()) \
}
...
TEST_METHOD(can_get_global_arrays)
{
    OPEN_LUA(lua)
    
    lua.setCode("X={1, 2, 3, 4, 5} Y={6, 7, 8, 9}");
    
    QString res = lua.execute();
    ASSERT_EQ_STR(res, "")
    
    auto resX = lua.getGlobalArray("X");
    ASSERT_IS_TRUE(resX.ok())
    QVector<int> x{1, 2, 3, 4, 5};
    ASSERT_EQ_LIST(resX.result(), x)

    auto resY = lua.getGlobalArray("Y");
    ASSERT_IS_TRUE(resY.ok())
    QVector<int> y{6, 7, 8, 9};
    ASSERT_EQ_LIST(resY.result(), y)
}
...
TEST_GROUP("LuaHelper",
    ...
    ADD_TEST(can_get_global_arrays),
)
